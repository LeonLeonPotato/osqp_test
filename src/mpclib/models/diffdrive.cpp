...
ADVec DifferentialDriveModel::autodiff(const ADVec& x, const ADVec& u) const {
    ADVec xdot(5);
    auto dvl = 0.5 * u[0] * params_.dt;
    auto dvr = 0.5 * u[1] * params_.dt;
    auto vl = x[3] + dvl;
    auto vr = x[4] + dvr;
...
Vec DifferentialDriveModel::infer(const Vec& x, const Vec& u) const {
    Vec xdot(5);
    float dvl = 0.5f * u[0] * params_.dt;
    float dvr = 0.5f * u[1] * params_.dt;
...
void DifferentialDriveModel::resync_from_params() {
    ...
    general_action_matrix(0, 0) = 1 / params_.acceleration_constant;
    general_action_matrix(1, 1) = 1 / params_.acceleration_constant;
